...
local roundNotifications = false
...
local disasterLabel = disasterSection:CreateLabel("Current Disaster: ") -- init later
...
local noiseTextBox = disasterSection:CreateTextbox("Notification Noise", "rbxassetid://")
noiseTextBox.FocusLost:Connect(function()
	roundNotificationSound = "rbxassetid://" .. noiseTextBox:GetText()
end)

miscSection:CreateButton("Disable Fall Damage", "Disable").Activated:Connect(function()
	local script = Players.LocalPlayer.Character:FindFirstChild("FallDamageScript")
	if script ~= nil then
		script:Destroy()
	end
end)
miscSection:CreateToggle("Show Compass", "Toggle", false).Toggled:Connect(function(state)
...
-- effectsSection:CreateToggle("Toggle", "Disable Sandstorm Effects", "Toggle", false).Toggled:Connect(function(state)
-- 	-- disable sandstorm effects
-- end)
-- effectsSection:CreateToggle("Toggle", "Disable Blizzard Effects", "Toggle", false).Toggled:Connect(function(state)
-- 	-- disable blizzard effects
-- end)
...
for _, sound in ipairs(workspace.ContentModel.Sounds:GetChildren()) do
	...
	soundsSection:CreateToggle('Disable "' .. sound.Name .. '"').Toggled:Connect(function(state)
		sound.Volume = state and 0 or originalVolume
	end)
end

-- disasterLabel:SetText("Current Disaster:" .. workspace.ContentModel.Information.Value)
-- workspace.ContentModel.Information.Changed:Connect(function(value)
-- 	disasterLabel:SetText("Current Disaster:" .. value)
-- end)
-- workspace.ContentModel.Status.Changed:Connect(function(value)
-- 	if value ~= "Survivers" then
-- 		return
-- 	end

-- 	-- coregui or something
-- 	local sound = Instance.new("Sound")
-- 	sound.SoundId = roundNotificationSound
